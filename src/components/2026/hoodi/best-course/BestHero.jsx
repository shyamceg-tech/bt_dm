/* HOODI BRANCH copy of BestHero.jsx — noindex Google-Ads page.
   Identical to the real BestHero (same Hero.module.css + shared HeroForm + the
   HERO copy from best-course.config) EXCEPT the location chip is localized to
   Hoodi / Hoodi Junction. The real best-course page's BestHero is untouched. */
...
/* Localize the one location chip to Hoodi; all other chips come straight from
   the shared config so this can never drift from the real page's copy. */
const CHIPS = HERO.chips.map((c) =>
  c.includes('Indiranagar') ? 'Hoodi, next to Hoodi Junction' : c,
);
